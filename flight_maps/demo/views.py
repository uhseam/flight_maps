...
def book_flight(request, flight):# pragma: no cover
...
def origin_airport_search(request):# pragma: no cover
    if request.is_ajax():
        try:
...
def destination_airport_search(request):# pragma: no cover
    if request.is_ajax():
        try:
            data = amadeus.reference_data.locations.get(
                keyword=request.GET.get("term", None), subType=Location.ANY
...
def get_city_airport_list(data):# pragma: no cover
    result = []
